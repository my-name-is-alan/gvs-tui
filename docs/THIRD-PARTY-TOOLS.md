...
Windows x64 的五个媒体工具作为普通二进制文件提交在项目根目录 `bin/`，随 `git clone/pull` 一起取得，不使用 LFS，也不生成便携包。版本、大小和 SHA-256 位于 `bin/manifest.json`；具体发行文件及源码来源见 [bin/README.md](../bin/README.md)，许可证原文位于 `bin/licenses/`。Bun 和 npm 依赖仍需安装。
